...
def draw_network_graph(n_nodes, seed_base):
    ...
    plt.savefig(f'logs/{POW_K}/network_topology_{n_nodes}_{seed_base}.png', bbox_inches='tight')
    plt.close()
...
def run_simulation(n_nodes, seed_base, pow_k=4):
    print(f"\n--- Starting simulation for N={n_nodes}, Seed={seed_base}, PoW K={pow_k} ---")
    clear_log()
    time.sleep(0.1)
    processes = []
    ...
    for i in range(n_nodes+1): # +1 for the bazri node
        port = BASE_PORT + i
        cmd = ["python", "node.py", "--port", str(port), "--seed", str(seed_base + i), "--pow-k", str(pow_k)]
        if i > 0:
    ...
    # Since nodes are solving PoW, they take longer to join.
    stabilize_wait = n_nodes//10 + (pow_k * 2)
    print(f"Waiting {stabilize_wait}s for nodes to solve PoW and stabilize...")
...
def main():
    results_file = open(f'logs/{POW_K}/simulation_results.txt', 'w')
    ...
    for n in NETWORK_SIZES:
        for run in range(RUNS_PER_SIZE):
            c_time, overhead = run_simulation(n, seed_base= 100 * n + run, pow_k= POW_K)
    ...
    plt.tight_layout()
    plt.savefig(f'logs/{POW_K}/phase3_results.png')
